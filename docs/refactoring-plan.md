...
> Phase 2（Product 責務分離）は大半完了。`titleForSearch` と Scrapbox placeholder formatter を pure domain function として `src/domain` に抽出し、`createScrapboxBodyString` を storage 非依存の同期 pure メソッド化、全角→半角変換を `src/domain/halfWidth.ts` に集約済み。残るは作業3（各 product class を `toTemplateVars()` + default format だけに寄せる）の仕上げのみ。
...
Phase 1 / Phase 1.5 / Phase 7a / Phase 6 / Phase 3 は完了済みとして扱う。最新の作業対象は Phase 2。
...
3. 各 product class は `toTemplateVars()` と default format を提供するだけに寄せる。（一部完了）
...
- `Product.ts` が `browser` を import しなくなったため、root `Product` 系に `products-must-not-import-storage-keys`（`chromeApi.ts` 禁止）と `products-must-not-import-extension-runtime`（`webextension-polyfill` 禁止）を追加済み。
