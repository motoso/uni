import type { FanzaDoujinScrapedData } from "./types";
import { createScraperLogger } from "./utils/logger";

const logger = createScraperLogger("FANZA-Doujin-Scraper");
 ...
 */
export function scrapeFanzaDoujinData(
  document: Document,
): FanzaDoujinScrapedData | null {
  logger.debug("Starting FANZA Doujin scraping...");
  ...
  if (titleElement) {
    ...
    const campaignSpan = clonedElement.querySelector(
      ".productTitle__txt--campaign",
    );
    if (campaignSpan) {
      campaignSpan.remove();
      logger.debug("Removed campaign span from title");
    }
  ...
  for (const selector of circleSelectors) {
    if (selector === ".circleName__txt") {
      circleNameElement = document.getElementsByClassName(
        "circleName__txt",
      )[0] as HTMLElement;
    } else {
    ...
    if (circleNameElement?.innerText?.trim()) {
      logger.debug(`Found circle name with selector: ${selector}`);
      break;
    }
    logger.debug(`Circle selector failed: ${selector}`);
  }

  const circleName = circleNameElement?.innerText?.trim() || "";
  if (circleName) {
    logger.debug(`Circle name found: ${circleName}`);
  } else {
    logger.debug("No circle name found");
  }

  // 作者名の取得：「作者」項目を探す
  let authors = [];
  const informationItems = document.getElementsByClassName(
  ...
  for (let i = 0; i < informationItems.length; i++) {
    ...
    if (titleElement && titleElement.innerText.trim() === "作者") {
      const txtElement = item.querySelector(
  ...
  const productInfoElements = document.getElementsByClassName(
    "productInformation__item",
  );
