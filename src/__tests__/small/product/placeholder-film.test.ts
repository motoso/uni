...
describe("Film placeholder replacement", () => {
  ...
  test("should replace film specific placeholders in custom format", () => {
    ...
    const body = film.createScrapboxBodyString({ film: customFormat });
    expect(body).toBe(
      "タイトル:テスト映画 ID:test_id_123 監督:[テスト監督] レーベル:[テストレーベル] 公開年:2023",
    );
  });

  test("should handle null director and label in custom format", () => {
    const film = Film.make(
      mockFilmData.service,
      mockFilmData.title,
      mockFilmData.actors,
  ...
  test("should use default format if custom format is not available", () => {
    ...
    expect(body).toContain(
      `[${mockFilmData.service}で視聴 ${mockFilmData.url}]`,
    );
  ...
  test("should handle null id in custom format (though _id is string, constructor allows null)", () => {
    const film = Film.make(
      mockFilmData.service,
      mockFilmData.title,
      mockFilmData.actors,
